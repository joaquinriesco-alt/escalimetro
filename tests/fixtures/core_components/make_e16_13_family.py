"""Familia de fixtures de E16.13 — un núcleo es UNA entidad que puede ocupar VARIAS regiones.

Las clases de este ciclo no preguntan "¿dónde está el núcleo?" sino "¿puede el motor REPRESENTAR el
núcleo que el dibujo tiene, y qué hace con las formas sucias?". Por eso seis de las diez declaran el
candidato en el JSON, igual que la familia de E16.11: lo que se prueba es la representación canónica
y el contrato, y hacerlos depender del productor convertiría un fallo de producción en un falso fallo
de contrato.
...
cantidad, área ni color proviene de un plano de desarrollo.
  ...
  A_SINGLE_COMPONENT_LEGACY     un bloque compacto: el caso degenerado de 1..N      -> accept, 1 pieza
  B_TWO_LEGITIMATE_COMPONENTS   dos bloques separados por circulación               -> accept, 2 piezas
  C_THREE_LEGITIMATE_COMPONENTS tres bloques repartidos en una banda                -> accept, 3 piezas
  D_FABRICATED_BRIDGE           candidato que cruza piso abierto para unir dos       -> fabricación alta
  E_SPURIOUS_COMPONENT          dos bloques + una pieza sobre papel en blanco        -> reject
  F_TINY_NOISE_COMPONENT        dos bloques + una esquirla bajo el piso de ruido     -> normaliza y cuenta
  G_OVERLAPPING_COMPONENTS      dos anillos que se solapan                           -> fusiona y cuenta
  H_COMPONENT_OUTSIDE_FOOTPRINT una pieza fuera de la huella                         -> InvalidCoreGeometry
  I_DISTRIBUTED_ANCHOR_COVERAGE anclas repartidas entre dos piezas                   -> COMPLETE sobre la unión
  J_OPEN_FLOOR_COMPONENT        dos bloques + una pieza que es piso ocupable         -> reject

Ejecutar: python tests/fixtures/core_components/make_e16_13_family.py
"""
import json
import os
...
def main():
    ...
    img = hoja(); planta(img); mobiliario(img); bloque(img, *DOS_A); bloque(img, *DOS_B)
    hechos.append(guardar("F_TINY_NOISE_COMPONENT", img, [440, 210, 800, 800], "normalize",
                          candidate_rings=[rect(465, 235, 775, 475), rect(465, 555, 775, 775),
                                           rect(830, 300, 848, 318)],
                          expected_components=2, expected_normalized_away=1))

    img = hoja(); planta(img); mobiliario(img); bloque(img, *DOS_A); bloque(img, *DOS_B)
    hechos.append(guardar("G_OVERLAPPING_COMPONENTS", img, [440, 210, 800, 800], "normalize",
                          candidate_rings=[rect(465, 235, 775, 475), rect(600, 400, 775, 620)],
                          expected_components=1, expected_merged_overlaps=1))
    ...
    bloque(img, *DOS_A, celdas=5); bloque(img, *DOS_B, celdas=5)
    hechos.append(guardar("I_DISTRIBUTED_ANCHOR_COVERAGE", img, [440, 210, 800, 800], "complete",
                          candidate_rings=[rect(465, 235, 775, 475), rect(465, 555, 775, 775)],
