"""E24 §16 — UNA lámina de contacto 3×3.
...
def celda(brief, alt):
    d = os.path.join(CASE, "layouts", f"E24_{brief}", "alternatives", alt)
    ...
    if os.path.exists(nf):
        ...
        razon = j.get("reason", razon)
        nota = j.get("note", "")
        if j.get("solver_status") == "INFEASIBLE":
            titulo = "NO_FIT"
            sub = "el solver PRUEBA que no hay solución sobre los candidatos generados"
        else:
            titulo = "SIN RESULTADO"
            sub = "el solver no concluyó en el tiempo del producto: no prueba nada"
    _txt(cell, titulo, (34, 96), 1.5, BAD, 3)
    if sub:
    ...
    y += 20
    _txt(cell, "No se fabrica un layout para llenar la grilla (E24 §16).", (34, y), 0.56, MUTED, 1)
    return cell
...
def main():
    ...
    _txt(board, "ESCALIMETRO  E24  ·  TRES BRIEFS x TRES ALTERNATIVAS  ·  OFICINA 403 (GPS PROPERTY)",
         (40, 58), 1.0, INK, 2)
    ...
    for i, b in enumerate(FILAS):
        info = ""
        p = os.path.join(CASE, "layouts", f"E24_{b}", "program_compiled.json")
        if os.path.exists(p):
    ...
    out = os.path.join(ROOT, "cases", "E24", "E24_CONTACT_SHEET_3x3.png")
